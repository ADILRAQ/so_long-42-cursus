...
/*   Updated: 2023/02/03 17:46:31 by araqioui         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	mlx_put_image_to_window(pl->mlx, pl->win, ig, pos.j * 64, pos.i * 64 + 64);
}
...
static void	ft_put_chars(char **map, t_info *pl)
{
	...
	while (map[++(pos.i)])
	{
		...
		while (map[pos.i][++(pos.j)])
		{
			...
			else if (map[pos.i][pos.j] == 'e')
				ft_img_to_window(pl, pl->ig.enemy, pos);
		}
	}
}
